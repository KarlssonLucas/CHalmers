...
public class Neighbours extends Application {
    ...
    void updateWorld() {
        final double threshold = 0.9;   // Percentage off neighbours that are equal to the position you're looking at
    ...
    void swapActor(Actor[][] worldCopy, Actor[][] world, double threshold, int rowLength) {
        ...
        for (int i = 0; i < rowLength; i++) {     // for-loop calculating row
            for (int k = 0; k < rowLength; k++) {     // for-loop calculating column
                ...
                if (!checkNeighbours (i, k, threshold)) { // Check how many percentage of the actors around you have the same value
                    swap (world, worldCopy, rowLength, i, k);
                }
            }
        }
    }

    void swap(Actor[][] world, Actor[][] worldCopy, int rowLength, int i, int k) {
        ...
        while (B != Actor.NONE) {           // while actor b is not equal to none, because we can only change current actor
            n = random.nextInt (rowLength);  // with empty actors. If B is not NONE we randomize n and n1 until we reach an empty actor
    ...
    private boolean checkNeighbours(int i, int i1, double t) {
        ...
        for (int dy = -1; dy <= 1; dy++) {
            for (int dx = -1; dx <= 1; dx++) {
                if (isValidLocation (world.length, i + dy, i1 + dx)) {
                    if (dy == 0 && dx == 0) { //Don't count the main actor as neighbor
                        continue;
                    } else {
                        if (world[i + dy][i1 + dx] == Actor.NONE) { // Don't count nones as neighbors
                            continue;
                        } else if (world[i + dy][i1 + dx] == A) { // If neighbor has the same value as main actor add 1 to sum
                            sum += 1;
                        }
                        div += 1;
                    }
        ...
        if (div == 0) return false; // div can't be zero
